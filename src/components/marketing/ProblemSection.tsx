...
export default function ProblemSection() {
  ...
  return (
    <section id="why" className="py-24 bg-gradient-to-b from-white to-neutral-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center mb-16">
          <p className="text-sm font-semibold text-[#FFCB05] uppercase tracking-wider mb-3">The Problem</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
          {problem.painPoints.map((point, index) => (
            <div
              key={index}
              className="flex items-center gap-4 p-5 rounded-2xl bg-white border border-neutral-100 shadow-sm hover:shadow-md transition-shadow"
            >
              <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-red-50 flex items-center justify-center">
                <PainIcon name={point.icon} />
              </div>
              <p className="text-neutral-700 font-medium text-sm">{point.text}</p>
            </div>
          ))}
        </div>
